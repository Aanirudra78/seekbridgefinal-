<?php
// view_student_profile.php - Universal student profile viewer (Industry + Institution)
...
if ($role === 'institution') {
    ...
    $instRow = mysqli_fetch_assoc($res);
    mysqli_stmt_close($st);
    $myCollege = $instRow ? $instRow['institution_name'] : '';
    if ($myCollege !== '' && $studentId > 0) {
        $st = db_query('SELECT user_id FROM student_details WHERE user_id = ? AND LOWER(TRIM(college_name)) = LOWER(?)', 'is', array($studentId, $myCollege));
        $res = mysqli_stmt_get_result($st);
        $allowed = mysqli_fetch_assoc($res) !== null;
        mysqli_stmt_close($st);
    }
} else {
    // industry: student must exist and have opted-in to data sharing
    if ($studentId > 0) {
        $st = db_query('SELECT user_id FROM student_details WHERE user_id = ? AND consent_share_data = 1', 'i', array($studentId));
        $res = mysqli_stmt_get_result($st);
        $allowed = mysqli_fetch_assoc($res) !== null;
        mysqli_stmt_close($st);
    }
...
if (!$allowed) {
    ...
    echo '<div class="alert alert-warning"><i class="fa-solid fa-shield-halved me-1"></i>Not authorized to view this student profile.</div>';
    echo '<a href="student_search.php" class="btn btn-outline-secondary btn-sm"><i class="fa-solid fa-arrow-left me-1"></i> Back to Search</a>';
...
// ---- Industry: applications/test results for THIS company ----
$apps = array();
if ($role === 'industry') {
    $st = db_query(
...
<a href="student_search.php" class="btn btn-outline-secondary btn-sm mb-3"><i class="fa-solid fa-arrow-left me-1"></i> Back to Search</a>
  ...
  <div class="col-lg-8">
    ...
    <div class="card mb-4">
      <div class="card-body">
        ...
        <?php if (count($apps) === 0): ?>
          <p class="text-muted-2 mb-0">This student hasn't applied to your postings yet.</p>
        <?php else: ?>
  ...
  <div class="col-lg-4">
    ...
    <div class="card">
      <div class="card-body">
        ...
        <?php else: ?>
          <p class="text-muted-2 small mb-0">Shortlist actions appear after the student applies to your postings.</p>
        <?php endif; ?>
      </div>
    </div>
    <?php endif; ?>
